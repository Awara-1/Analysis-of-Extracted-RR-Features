...
def display_classification_report(y_test, y_pred):
    try:
        return classification_report(y_test, y_pred, target_names=['0: Alert', '1: Death', '2: Home', '3: Hospitalisation', '4: No Outcome'])
    except:
        return classification_report(y_test, y_pred, target_names=['1: Death', '2: Home', '3: Hospitalisation', '4: No Outcome'])
...
def find_best_model(X, y, model, kf, scorer):
    ...
    grid = GridSearchCV(estimator, params, cv=kf, verbose=0, scoring='accuracy')
    ...
    model_test_score = cohen_kappa_score(y_predict, y_test)
...
models = [LogisticRegression(max_iter=10000), DecisionTreeClassifier(criterion = "gini",
...
for model in models:
    y_predict, y_test, model_test_score, model_name, best_cv_score, best_params = find_best_model(X, y, model, kf, kappa_scorer)
